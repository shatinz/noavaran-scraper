...
ARCHITECT_PATTERNS = [
    r'(?:طراح(?:ی)?|آرشیتکت|مهندسین\s+مشاور|معمار|دفتر\s+معماری|طراحی\s+معماری)\s*[:\-–—]?\s*([^,\n\.\;\|]+)',
...
CONTRACTOR_PATTERNS = [
    r'(?:مجری|پیمانکار|سازنده|سازه|کارفرما\s+و\s+مجری|مجری\s+ذیصلاح|اجرای\s+سازه|اجرای\s+نما)\s*[:\-–—]?\s*([^,\n\.\;\|]+)',
...
def extract_architects(text: str) -> List[str]:
    ...
    for pat in ARCHITECT_PATTERNS:
        for m in re.findall(pat, text):
            cand = re.split(r'\s+(?:و|با|در|برای|به)\s+', m.strip())[0].strip()
            cand = re.sub(r'[\:\-–—\.\,]+$', '', cand).strip()
            if len(cand) >= 4 and cand not in seen and not any(k in cand for k in ['پروژه', 'احداث', 'مترمربع']):
                seen.add(cand)
                results.append(cand)
    return results
